    ...
    for iteration, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.amp.autocast("cuda"):
            ...
            if not args.cosub:
                ...
                # 1. Softmax 계산
                # if device_id == 0: 
                #     outputs_softmax = F.softmax(outputs, dim=1).detach().cpu().numpy()
                #     targets_np = targets.detach().cpu().numpy()

                #     # 2. NaN 또는 Inf 검사 (Softmax Outputs)
                #     if not np.isfinite(outputs_softmax).all():
                #         print("!!!!! Warning: NaN or Inf values detected in the softmax outputs!")
                #         # NaN 또는 Inf를 제거하기 위해 대체값 삽입 (예: 0.0)
                #         outputs_softmax = np.nan_to_num(outputs_softmax, nan=0.0, posinf=0.0, neginf=0.0)

                #     # 3. Softmax 결과 저장
                #     data_to_save = {"Outputs": [list(row) for row in outputs_softmax]}  # 행 단위로 리스트로 저장
                #     df = pd.DataFrame(data_to_save)

                #     # Ensure output directory exists
                #     output_dir = "output_softmax_probing"
                #     os.makedirs(output_dir, exist_ok=True)

                #     # Save softmax outputs to CSV
                #     softmax_file_name = os.path.join(output_dir, f"iteration_{epoch * 1251 + iteration}_outputs_softmax.csv")
                #     df.to_csv(softmax_file_name, index=False)
                #     # print(f"Softmax outputs saved to {softmax_file_name}")

                #     # 4. Softmax - Targets 계산
                #     difference = outputs_softmax - targets_np

                #     # 5. NaN 또는 Inf 검사 (Difference)
                #     if not np.isfinite(difference).all():
                #         print("QWarning: NaN or Inf values detected in the difference (Softmax - Targets)!")
                #         # NaN 또는 Inf를 제거하기 위해 대체값 삽입
                #         difference = np.nan_to_num(difference, nan=0.0, posinf=0.0, neginf=0.0)

                #     # 6. 차이값 저장
                #     diff_data_to_save = {f"Diff_{i}": difference[:, i].tolist() for i in range(difference.shape[1])}
                #     diff_df = pd.DataFrame(diff_data_to_save)

                #     # Save differences to another CSV
                #     diff_file_name = os.path.join(output_dir, f"iteration_{epoch * 1251 + iteration}_grads.csv")
                #     diff_df.to_csv(diff_file_name, index=False)
                # print(f"Differences (Softmax - Targets) saved to {diff_file_name}")

                # if iteration == 0:
                #     outputs_np = outputs.detach().cpu().numpy()
                #     targets_np = targets.detach().cpu().numpy()

                #     # Combine outputs and targets for saving
                #     data_to_save = {
                #         "Outputs": outputs_np.tolist(),
                #         "Targets": targets_np.tolist(),
                #     }

                #     # Convert to a DataFrame for structured saving
                #     df = pd.DataFrame(data_to_save)
                    
                #     output_dir = "output_probing"
                #     os.makedirs(output_dir, exist_ok=True)  

                #     file_name = os.path.join(output_dir, f"epoch_{epoch}.csv")
